...
export function RandomLetterSwapForward({
  ...
  const mergeTransition = (t: AnimationOptions, i: number) => ({
    ...t,
  ...
  const hoverStart = debounce(
    () => {
      ...
      for (let i = 0; i < label.length; i++) {
        ...
        animate(".letter-" + randomIndex, { y: reverse ? "100%" : "-100%" }, mergeTransition(transition, i)).then(
          () => {
...
export function RandomLetterSwapPingPong({
  ...
  const hoverEnd = debounce(
    () => {
      ...
      for (let i = 0; i < label.length; i++) {
        const randomIndex = shuffledIndices[i];
        animate(".letter-" + randomIndex, { y: 0 }, mergeTransition(transition, i));
        animate(
          ".letter-secondary-" + randomIndex,
          { top: reverse ? "-100%" : "100%" },
